...
data(airquality)
fit2=gam(Ozone^(1/3) ~ lo(Solar.R) + lo(Wind, Temp), data=airquality, subset=!is.na(Ozone))
...
## The next bit of code is irritating - it is because some versions of R have changes in family
fit1$family <-NULL
fit2$family <-NULL
fit3$family <-NULL
fit4$family <-NULL
...
objects  <- list(
    ...
    step1=step1,
    pred1=pred1)
###saveRDS(objects, "tests/testthat/test_results/gam-results.RDS")

expected  <- readRDS("test_results/gam-results.RDS")
